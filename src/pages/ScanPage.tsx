import { ChangeEvent, useRef, useState } from "react";
import { Camera, ChevronRight, ImagePlus, LayoutTemplate, ScanLine } from "lucide-react";
import LiveScanner from "../components/LiveScanner";
import { AnswerCardTemplate, Recognition, recognizeAnswerCard } from "../lib/omr";
import EmptyState from "./EmptyState";

type Props = {
  template: AnswerCardTemplate | null;
  onSelect: () => void;
  onScanned: (recognition: Recognition, fileName: string) => void;
  notify: (text: string) => void;
};
export default function ScanPage({ template, onSelect, onScanned, notify }: Props) {
  const fileRef = useRef<HTMLInputElement>(null);
  const [processing, setProcessing] = useState(false);
  const [scanner, setScanner] = useState(false);
  const importImage = (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file || !template) return;
    setProcessing(true);
    const url = URL.createObjectURL(file);
    const image = new Image();
    image.addEventListener(
      "load",
      () => {
        try {
          onScanned(
            recognizeAnswerCard(image, image.naturalWidth, image.naturalHeight, template),
            file.name,
          );
        } finally {
          URL.revokeObjectURL(url);
          setProcessing(false);
        }
      },
      { once: true },
    );
    image.addEventListener(
      "error",
      () => {
        URL.revokeObjectURL(url);
        setProcessing(false);
        notify("图片无法读取");
      },
      { once: true },
    );
    image.src = url;
  };
  if (scanner && template)
    return (
      <LiveScanner
        template={template}
        onClose={() => setScanner(false)}
        onConfirm={(recognition) => {
          setScanner(false);
          onScanned(recognition, `camera-${Date.now()}.jpg`);
        }}
      />
    );
  return (
    <>
      <header className="page-top scan-top">
        <div>
          <h1>扫描答卷</h1>
          <p>
            {template ? `${template.name} · ${template.questionCount} 道题` : "请先选择答题卡模板"}
          </p>
        </div>
      </header>
      <main className="page scan-page">
        {template ? (
          <>
            <button onClick={onSelect} className="exam-picker">
              <div className="exam-icon">
                <LayoutTemplate size={21} />
              </div>
              <div>
                <small>当前答题卡</small>
                <b>{template.name}</b>
                <span>
                  {template.subject} · {template.questionCount} 道单选题
                </span>
              </div>
              <ChevronRight size={19} />
            </button>
            <section className="scan-zone">
              <div className="scan-frame">
                <span></span>
                <span></span>
                <span></span>
                <span></span>
                <Camera size={38} />
              </div>
              <h2>实时扫描答题卡</h2>
              <p>将系统生成答题卡的四个定位方块完整放入画面，系统会实时显示对错浮层。</p>
              <button onClick={() => setScanner(true)} className="scan-button">
                <Camera size={20} />
                打开实时相机
              </button>
              <button onClick={() => fileRef.current?.click()} className="album-button">
                <ImagePlus size={19} />
                导入已拍摄答题卡
              </button>
              <input ref={fileRef} onChange={importImage} type="file" accept="image/*" hidden />
            </section>
            {processing && (
              <p className="scan-tip">
                <ScanLine size={15} />
                正在读取答题卡真实填涂结果
              </p>
            )}
            <p className="real-note">
              导入图片只适用于正面、完整、未裁切的答题卡。倾斜拍摄请使用实时相机模式。
            </p>
          </>
        ) : (
          <EmptyState onCreate={onSelect} />
        )}
      </main>
    </>
  );
}
