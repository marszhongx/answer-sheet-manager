...
import { AnswerCardTemplate, createLayout, OPTION_LABELS, Recognition, recognizeWarpedCard } from '../lib/omr'
...
function chooseCorners(points: Point[]): Point[] | null {
  ...
  const unique = (point: Point, used: Point[]) => !used.some(item => Math.hypot(item.x - point.x, item.y - point.y) < 12)
  const select = (sort: (a: Point, b: Point) => number, used: Point[]) => points.slice().sort(sort).find(point => unique(point, used))
...
export default function LiveScanner({ template, onConfirm, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function start() {
      try {
        ...
        setState('error')
        setMessage(error instanceof Error ? `无法打开相机：${error.message}` : '无法打开相机，请检查权限')
      }
    }
    start()
    return () => { disposed = true; if (requestRef.current) cancelAnimationFrame(requestRef.current); streamRef.current?.getTracks().forEach(track => track.stop()) }
  }, [])
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      ...
      if (!video || !frame || !overlay || video.readyState < HTMLMediaElement.HAVE_CURRENT_DATA) return
      const width = 720
  ...
  return <div className="scanner-screen">
    ...
    <header className="scanner-header"><button aria-label="关闭相机" onClick={onClose}><X size={22} /></button><div><b>实时扫描</b><span>{state === 'ready' ? '答题卡已定位' : '定位答题卡'}</span></div><button aria-label="切换为重新定位" onClick={() => { setRecognition(null); setState('searching') }}><RotateCcw size={20} /></button></header>
    <div className={`scanner-status ${state}`}>
      {state === 'loading' ? <LoaderCircle className="spin" size={17} /> : state === 'ready' ? <Check size={17} /> : <ScanLine size={17} />}<span>{message}</span>
    </div>
    {recognition && <div className="scanner-score"><b>{recognition.answers.filter((answer, index) => answer === template.answers[index]).length * 5}</b><span>/ {template.questionCount * 5} 分</span></div>}
    <footer className="scanner-footer"><button className="scanner-capture" disabled={!recognition || !recognition.markerValid} onClick={() => recognition && onConfirm(recognition)}><Camera size={21} />{recognition?.markerValid ? '确认本次阅卷' : '正在寻找答题卡'}</button></footer>
  </div>
}
...
async function processFrame(
  ...
  try {
    ...
    for (let index = 0; index < contours.size(); index++) {
      ...
      if (rect.width < 11 || rect.height < 11 || rect.width > 90 || rect.height > 90 || ratio < 0.72 || ratio > 1.28) continue
    ...
    const destination = layout.markers.map(marker => ({ x: marker.x + marker.size / 2, y: marker.y + marker.size / 2 }))
    const sourcePoints = cv.matFromArray(4, 1, cv.CV_32FC2, corners.flatMap(point => [point.x, point.y]))
    const targetPoints = cv.matFromArray(4, 1, cv.CV_32FC2, destination.flatMap(point => [point.x, point.y]))
    ...
    cv.warpPerspective(full, warped, transform, new cv.Size(layout.width, layout.height), cv.INTER_LINEAR, cv.BORDER_CONSTANT, new cv.Scalar(255, 255, 255, 255))
    const warpedCanvas = document.createElement('canvas')
    warpedCanvas.width = layout.width; warpedCanvas.height = layout.height
    ...
    const recognition = recognizeWarpedCard(warpedContext.getImageData(0, 0, layout.width, layout.height), template)
    ...
    sourcePoints.delete(); targetPoints.delete(); transform.delete(); inverse.delete(); full.delete(); warped.delete()
    setRecognition(recognition)
    setState('ready')
    setMessage(recognition.answers.some(answer => answer === null) ? '检测到未填或多填项，请检查标记' : '识别稳定，可确认阅卷')
  } catch {
    setState('searching')
    setRecognition(null)
...
function drawOverlay(overlay: CanvasRenderingContext2D, corners: Point[], layout: ReturnType<typeof createLayout>, inverse: number[], recognition: Recognition, template: AnswerCardTemplate) {
  overlay.lineWidth = 3
  ...
  layout.bubbles.filter(bubble => recognition.answers[bubble.question] === null).filter(bubble => bubble.option === 'A').forEach(bubble => {
    const point = project({ x: bubble.x, y: bubble.y }, inverse)
