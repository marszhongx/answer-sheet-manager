...
export type CardLayout = { width: number; height: number; markers: Array<{ x: number; y: number; size: number }>; bubbles: Bubble[] }
export type Recognition = { answers: Array<Option | null>; confidence: number[]; fillRates: number[][]; markerValid: boolean }
...
export function createLayout(questionCount: number): CardLayout {
  ...
  const markerPositions = [
    { x: MARGIN, y: MARGIN }, { x: CARD_WIDTH - MARGIN - MARKER_SIZE, y: MARGIN },
    { x: MARGIN, y: height - MARGIN - MARKER_SIZE }, { x: CARD_WIDTH - MARGIN - MARKER_SIZE, y: height - MARGIN - MARKER_SIZE },
  ]
  ...
  for (let question = 0; question < questionCount; question++) {
    ...
    const y = 304 + row * 54
    OPTION_LABELS.forEach((option, index) => bubbles.push({
      question, option, x: originX + 58 + index * 44, y, radius: 15,
    }))
  }
  return { width: CARD_WIDTH, height, markers: markerPositions.map(marker => ({ ...marker, size: MARKER_SIZE })), bubbles }
}
...
export function drawAnswerCard(canvas: HTMLCanvasElement, template: AnswerCardTemplate) {
  ...
  layout.bubbles.forEach(bubble => groups.set(bubble.question, [...(groups.get(bubble.question) ?? []), bubble]))
...
function cropAndScale(image: CanvasImageSource, sourceWidth: number, sourceHeight: number, target: CardLayout): ImageData {
...
function darkness(data: ImageData, x: number, y: number, radius: number): number {
  ...
  for (let py = minY; py <= maxY; py++) for (let px = minX; px <= maxX; px++) {
    ...
    const luminance = data.data[index] * 0.299 + data.data[index + 1] * 0.587 + data.data[index + 2] * 0.114
...
export function classifyFillRates(fillRates: number[][]): Pick<Recognition, 'answers' | 'confidence'> {
  ...
  fillRates.forEach(rates => {
    ...
    answers.push(top.rate >= 0.18 && top.rate - second.rate >= 0.06 ? OPTION_LABELS[top.index] : null)
    confidence.push(Math.max(0, Math.min(1, (top.rate - second.rate) / 0.25)))
  })
  return { answers, confidence }
}

export function recognizeWarpedCard(imageData: ImageData, template: AnswerCardTemplate, markerValid = true): Recognition {
  const layout = createLayout(template.questionCount)
  const fillRates = Array.from({ length: template.questionCount }, () => Array(4).fill(0))
  layout.bubbles.forEach(bubble => { fillRates[bubble.question][OPTION_LABELS.indexOf(bubble.option)] = darkness(imageData, bubble.x, bubble.y, bubble.radius * 0.57) })
  return { ...classifyFillRates(fillRates), fillRates, markerValid }
}

export function recognizeAnswerCard(image: CanvasImageSource, sourceWidth: number, sourceHeight: number, template: AnswerCardTemplate): Recognition {
  const layout = createLayout(template.questionCount)
