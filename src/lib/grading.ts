...
export function questionRates(records: GradedStudent[], questionCount: number): number[] {
  ...
  return Array.from({ length: questionCount }, (_, index) => Math.round(records.filter(record => !record.wrong[index]).length / records.length * 100))
}
...
export function toCSV(template: AnswerCardTemplate, records: GradedStudent[]): string {
  const header = ['姓名', ...Array.from({ length: template.questionCount }, (_, index) => `第${index + 1}题`), '得分', '总分']
  const rows = records.map(record => [record.name, ...record.answers.map(answer => answer ?? '未识别'), record.score, record.totalScore])
